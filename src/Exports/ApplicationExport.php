...
namespace Quinton\Applications\Exports;
...
class ApplicationExport extends TableExportHandler
{
    protected function afterSheet(AfterSheet $event)
    {
        ...
        for ($index = 2; $index <= $totalRows; $index++) {
            ...
            if ($value == ApplicationStatusEnum::NEW) {
            ...
            $event->sheet
                ->getDelegate()
                ->getCell('F' . $index)
                ->setValue(ApplicationStatusEnum::getLabel($value));
        }
    }
}
